...
class Network:
    ...
    def construct(self, args):
        with self.session.graph.as_default():
            ...
            # init_state = rnn_cell.zero_state(args.batch_size, dtype=tf.float32)
            hidden_layer, state = tf.nn.dynamic_rnn(rnn_cell, self.sequences, dtype=tf.float32)
            ...
            y = tf.zeros(tf.shape(output_layer), tf.float32)
            self.predictions = tf.greater_equal(output_layer, y)
