...
class Network:
    ...
    def construct(self, args, source_chars, target_chars, bow, eow):
        with self.session.graph.as_default():
            ...
            if args.bn_src:
                source_encoded = tf.layers.batch_normalization(source_encoded, training=self.is_training, name='bn_src')
            ...
            if args.bn_tar:
                embedded_target = tf.layers.batch_normalization(embedded_target, training=self.is_training, name='bn_tar')
            ...
            class DecoderPrediction(tf.contrib.seq2seq.Decoder):
                ...
                def step(self, time, inputs, states, name=None):
                    ...
                    next_inputs = with_attention(next_inputs, states)
                    # Checkc if done. True where outputs==eow, False otherwise
...
if __name__ == "__main__":
    ...
    parser.add_argument("--char_dim", default=128, type=int, help="Character-level embedding dimension.")
    ...
    parser.add_argument("--bn_src", default=False, type=bool, help="Batch normalization.")
    parser.add_argument("--bn_tar", default=False, type=bool, help="Batch normalization.")
    ...
    # Predict dev data
    with open("{}/lemmatizer_sota_dev.txt".format(args.logdir), "w") as test_file:
