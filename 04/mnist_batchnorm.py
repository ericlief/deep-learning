...
class Network:
    ...
    def construct(self, args):
        with self.session.graph.as_default():
            ...
            for layer in layers:
                ...
                # Batch normalization layer CB-filters-kernel_size-stride-padding
                elif opts[0] == 'CB':  # e.g. CB-10-3-2-same
                    n_conv_layers += 1
                    n_filters = int(opts[1]) # e.g. 10
                    filter_size = int(opts[2]) # e.g. 3 X 3 (sqr)
                    filter_shape = [filter_size, filter_size]  # e.g. [3, 3]                   
                    ...
                    # add conv layer: 
                    # args:           add_conv_layer(input_data, n_input_channels, n_filters, filter_shape, stride_shape, pad, name):
                    # out_layer = self.add_conv_layer(self.images, 1, n_filters, filter_shape, stride_shape, pad, activation=None, name='conv_layer'+str(n_conv_layers))
                    out_layer = tf.layers.conv2d(out_layer, n_filters, filter_size, strides, padding=pad, activation=None, use_bias=False, name='conv_layer_BN_'+str(n_conv_layers))
                    out_layer = tf.layers.batch_normalization(out_layer, training=self.is_training, name='conv_layer_BN_'+str(n_conv_layers))
                    out_layer = tf.nn.relu(out_layer, name='conv-relu'+str(n_conv_layers))
    ...
    def add_conv_layer(self, input_data, n_input_channels, n_filters, filter_shape, stride_shape, pad, activation, name):
        ...
        if activation is None:
            ...
            conv_layer = tf.nn.relu(conv_layer, name=name+'-relu')
        elif activation == "tanh":
            conv_layer = tf.tanh(conv_layer, name=name+'-tanh')
        elif activation == "sigmoid":
            conv_layer = tf.sigmoid(conv_layer, name=name+'-sigmoid')
        else:
    ...
    def train(self, images, labels):
        self.session.run([self.training, self.summaries["train"]], feed_dict={self.images: images, self.labels: labels, self.is_training: True})

    def evaluate(self, dataset, images, labels):
        accuracy, _ = self.session.run([self.accuracy, self.summaries[dataset]], feed_dict={self.images: images, self.labels: labels, self.is_training: False})
        return accuracy
