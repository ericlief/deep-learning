        ...
        with self.session.graph.as_default():
            ...
            with tf.name_scope('cle'):
                ...
                if args.dropout_char:
                    cell_fw = tf.nn.rnn_cell.DropoutWrapper(cell_fw, input_size=embedded_chars.get_shape()[-1], input_keep_prob=1-args.dropout_char, output_keep_prob=1-args.dropout_char, variational_recurrent=True, dtype=tf.float32)
                    cell_bw = tf.nn.rnn_cell.DropoutWrapper(cell_bw, input_size=embedded_chars.get_shape()[-1], input_keep_prob=1-args.dropout_char, output_keep_prob=1-args.dropout_char, variational_recurrent=True, dtype=tf.float32)            
                ...
                outputs, states = tf.nn.bidirectional_dynamic_rnn(cell_fw, cell_bw, inputs=embedded_chars, sequence_length=self.charseq_lens, dtype=tf.float32, scope="cle")
                
                # Sum hidden states (fw + bw)             
                char_states = tf.reduce_sum(states, axis=0)
