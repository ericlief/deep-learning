...
class Network:
    ...
    def construct(self, args):
        ...
        with self.session.graph.as_default():
            ...
            def generator(z):
                ...
                # Generates pixels: sigmoid guarantees output in range [0,1] -> logits of pixels
                output = tf.layers.dense(z, 128, tf.nn.relu)
                logits = tf.layers.dense(output, self.HEIGHT * self.WIDTH, tf.sigmoid)  # [?, H*W]
            ...
            with tf.variable_scope("generator"):
                ...
                # This training of G is independent from that of D (different scopes)
    ...
    #def generate(self):
        ...
        #else:
            ## Generate random Z
            #starts, ends = self.sample_z(GRID), self.sample_z(GRID) # (GRID/B, zdim) - > (20, 100)
        
        #print('s',starts.shape)
        #print('e',ends.shape)
        #interpolated_z = []
        #for i in range(GRID):
            #print('grids', (starts[i] + (ends[i] - starts[i])).shape)
            #print('* expand', np.expand_dims(np.linspace(0, 1, GRID), -1).shape)
            #print('=', (starts[i] + (ends[i] - starts[i]) * np.expand_dims(np.linspace(0, 1, GRID), -1)).shape)
            #interpolated_z.extend(starts[i] + (ends[i] - starts[i]) * np.expand_dims(np.linspace(0, 1, GRID), -1))
            
            ## ^ = row(i) of (20, 100) - > dim 100
        ##print('interp', interpolated_z.shape)
        #interpolated_images = self.session.run(self.generated_images, {self.z: interpolated_z})
        #print('interp img', interpolated_images.shape)
