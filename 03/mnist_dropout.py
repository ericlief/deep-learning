...
class Network:
    ...
    def construct(self, args):
        with self.session.graph.as_default():
            ...
            #hidden_layer_dropout = tf.cond(self.is_training, 
                                   #lambda: tf.nn.dropout(hidden_layer, keep_prob=args.dropout), # func 1 if true
    ...
    def train(self, images, labels):
        #self.is_training = self.session.run(self.is_training, True) 
    ...
    def evaluate(self, dataset, images, labels):
        #self.is_training = self.session.run(self.is_training, False)         
...
if __name__ == "__main__":
    ...
    print("{:.2f}".format(100 * accuracy[1]))
    #print(accuracy) 